...
ASSET_PATH = os.path.join(BASE_PATH, "assets", "sawyer.xml")
...
class Reacher7DOFEnv(MujocoEnv, utils.EzPickle):
    ...
    def reward_fn(self, obs, act, obs_next):
        obs_next = jnp.clip(obs, -10.0, 10.0)
        hand_pos = obs_next[-6:-3]
        target_pos = obs_next[-3:]
