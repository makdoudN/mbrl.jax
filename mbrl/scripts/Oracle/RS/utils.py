...
from mbrl.common.nn import mlp_categorical
...
@partial(jit, static_argnums=(3, 6, 7, 8))
def fmodel_inference(
        rng, observation, action,
        forward_def, params,
        norm_state, model,
        use_residual,
        action_type):
    """ Generic forward inference function.

    Implements the forward model after the training of the model.
    It handle `deterministic` and `stochastic` model.
    """
    ...
    if action_type == 'discrete':
        return prediction.sample(seed=rng)
...
def init_fmodel_training(
    ...
    if fmodel_type == 'D':
        fmodel_cls = mlp_deterministic
    if fmodel_type == 'P' and action_type == 'continuous':
        fmodel_cls = mlp_multivariate_normal_diag
    if fmodel_type == 'P' and action_type == 'discrete':
        fmodel_cls = mlp_categorical
...
def train_fmodel(
    ...
    if use_norm and action_type == 'continuous':
