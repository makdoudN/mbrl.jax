...
@hydra.main(config_path="conf", config_name="rs")
def main(conf):
    ...
    score_ = functools.partial(
        score_trajectory,
        discount=conf.discount,
        terminal_reward_fn=None
    )

    plan_ = functools.partial(
        plan,
        population=conf.rs.population,
        forecast=forecast_, score=score_
    )
    policy = jit(lambda rng, x: plan_(rng, x)[0][0])

    @jit
    def one_step_interaction(carry, t):
        rng, env_state = carry
        action = policy(rng, env_state)
        env_state_next = env.step(env_state, action)
        carry = (rng, env_state_next)
        return carry, {'reward': env_state_next.reward}
